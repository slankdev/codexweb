...
# 5. ローカル DB を立てる (docker)
docker run -d --name codexweb-mysql -p 3306:3306 \
  -e MYSQL_ROOT_PASSWORD=root \
  -e MYSQL_DATABASE=codexweb \
  -e MYSQL_USER=codexweb -e MYSQL_PASSWORD=codexweb \
  mysql:8

# 6. .env.local に DATABASE_URL を追記
echo 'DATABASE_URL=mysql://codexweb:codexweb@127.0.0.1:3306/codexweb' >> .env.local
...
| `DATABASE_URL` | **必須**。MySQL 接続文字列。`mysql://user:pass@host:3306/db` (TCP) または `mysql://user:pass@localhost/db?socketPath=/cloudsql/PROJECT:REGION:INSTANCE` (Cloud SQL socket)。 |
...
`main` への push で `.github/workflows/cloudrun.yml` が起動し、

1. Cloud SQL Auth Proxy 経由で **MySQL マイグレーション**を適用
2. Artifact Registry に **イメージを build & push**
3. **Cloud Run にデプロイ** (Cloud SQL の Unix socket を attach)

の順で走ります。**シングルインスタンス前提** で動作するよう設定済み
(`min-instances=1`, `max-instances=1`, `--timeout=3600`) です。
タスクとチャット履歴は Cloud SQL に永続化されるので、リビジョン入れ
替えでも保持されます。
...
# Cloud Run のランタイム SA (= デフォルトの Compute SA) にも Cloud SQL Client
...
# Cloud SQL for MySQL インスタンス (db-f1-micro = 月 ~$10)
SQL_INSTANCE=codexweb-mysql
...
# Instance Connection Name は <project>:<region>:<instance> 形式
ICN="$PROJECT_ID:$REGION:$SQL_INSTANCE"
...
# DB 接続文字列 (Runtime: Cloud SQL socket / Migrate: Cloud SQL Auth Proxy 経由)
printf "%s" "mysql://$DB_USER:$DB_PASS@localhost/$DB_NAME?socketPath=/cloudsql/$ICN" | \
  gcloud secrets create DATABASE_URL --data-file=- --project="$PROJECT_ID"
printf "%s" "mysql://$DB_USER:$DB_PASS@127.0.0.1:3306/$DB_NAME" | \
  gcloud secrets create DATABASE_URL_MIGRATE --data-file=- --project="$PROJECT_ID"
```
...
| `CLOUD_SQL_INSTANCE` | `<PROJECT_ID>:<REGION>:codexweb-mysql` (= `$ICN`) |
...
> `CLOUD_SQL_INSTANCE` を空のままにすると DB マイグレーションと Cloud SQL
> アタッチがスキップされ、デプロイ自体は通るがアプリは起動直後に
> `DATABASE_URL is not set` で落ちます。必ずセットしてください。
...
- **永続化**: タスク本体とチャット履歴 (events) は Cloud SQL に保存
  されるので、デプロイ間で残ります。一方で **実行中の codex プロセス**
  はリビジョン入れ替えで死ぬので、その時点で `running` だったタスクは
  起動時に `stopped` にマークされて続行不可になります (DB 上の履歴は
  残るので「失われた」感は薄い)。
- `--timeout=3600` を指定しているので Cloud Run としては 60 分まで SSE が
  維持されます。それ以上のタスクは途中で切れるので、ローカル/専用 VM での
  運用を推奨。
- Cloud SQL `db-f1-micro` は **共有 CPU** で、codex のイベント書き込み
  が重い時に瞬間的に詰まる可能性があります (1 イベント 1 INSERT ではなく
  100ms バッファで bulk insert する設計なので通常は問題なし)。重い使い方
  をするなら `db-g1-small` 以上に上げてください。
