...
**初回だけ手動作業**: bot コメントに表示される redirect URI
(`https://<preview-url>/api/auth/callback`) を Google OAuth client の
"Authorized redirect URIs" に追加して Save してください (PR 番号ごとに URL が
変わるので、新しい PR を出すたび追加が必要)。
